            ...
            <div class="carousel" ref="carousel" v-if="heroContents.length == 20">
                <div class="card" v-for="(content, index) in contents" :key="index">
                    ...
                    <div class="info">
                        ...
                        <div class="content-title" @click="openInfo(content)" @mouseover="stopCarousel()" @mouseleave="startCarousel()" :title="content.title || content.name">
                            <img v-if="content.logo != null" :src="'https://image.tmdb.org/t/p/original/' + content.logo" alt="">
                        ...
                        <div class="rank">
                            ...
                            <p>Oggi al n° {{index + 1}} fra {{data.selected == data.pages[0] ? 'i' : data.selected == data.pages[1] ? 'le serie' : 'i film'}} più popolari</p>
                        </div>
                        <div class="overview">
                            <p>{{content.overview}}</p>
                        </div>
                        <div class="info-btn" @mouseover="stopCarousel()" @mouseleave="startCarousel()" @click="openInfo(content)" title="Mostra informazioni">
                            <div class="circle">
                                <span>i</span>
                            </div>
                            <span>Mostra info</span>
                        </div>
                    </div>
                </div>
            </div>
...
export default {
    ...
    methods: {
        ...
        carousel() {
            this.startCarousel();
            this.$refs.carousel.addEventListener('transitionend', (e) => {
                if (e.target == this.$refs.carousel) {
                    ...
                    this.$refs.carousel.style.transform = 'translate(0)';
                    setTimeout(() => {
                        this.$refs.carousel.style.transition = 'all 2s';
                    }, 200);
                }
            });
        },
    ...
    mounted() {
        const waitForCarousel = () => {
            if (this.$refs.carousel) {
                this.carousel();
            } else setTimeout(waitForCarousel, 250);
        }
        waitForCarousel();
    },
    computed: {
        contents() {
            let array = this.heroContents.filter(content => {
                if (content.overview) {
                    return content;
                }
            });
            ...
            .card {
                ...
                .background-img {
                    ...
                    .layover {
                        width: 100%;
                        height: 100%;
                        background: linear-gradient(rgba(0,0,0,0) 40%, rgba(20,20,20,1) 92%);
                        @media screen and (min-width: 768px) {
                            background: linear-gradient(rgba(0,0,0,0) 40%, rgba(20,20,20,1) 92%),
                                linear-gradient(to left, rgba(0,0,0,0) 40%, rgba(6,6,6,.9) 100%);
                        }
                    }
                }
                .info {
